...
namespace Mobillogix\JobsBundle\Tests\Job;
...
use Mobillogix\JobsBundle\Tests\Stub\StubJob;
...
class JobPersistenceServiceTest extends BaseJobsTestCase
{
    ...
    protected $config = [
        'types' => [
            'test' => [
                'name'  => 'Test',
                'priority'  => 0,
                'job_class' => 'Mobillogix\JobsBundle\Tests\Stub\StubJob',
                'process_class' => 'Mobillogix\JobsBundle\Tests\Stub\StubJob',
                'packages_chunk' => 10,
    ...
    public function testShouldAddJob()
    {
        ...
        $em = $this->getEntityManagerMock([
            'MobillogixJobsBundle:JobType' => $jobTypeRepository,
            'MobillogixJobsBundle:Job' => $jobRepository,
        ]);
    ...
    /**
     * @expectedException \Exception
     * @expectedExceptionMessage Job class 'Mobillogix\JobsBundle\Tests\Job\JobPersistenceServiceTest' must be subclass of Mobillogix\JobsBundle\Model\BaseJob
     */
    public function testShouldThrowWrongParentClassException()
    {
        ...
        $jobRepository = $this->getJobRepositoryMock(['findJobsForPlanning']);
        $em = $this->getEntityManagerMock([
            1 => ['MobillogixJobsBundle:Job', $jobRepository],
        ]);
    ...
    public function testShouldMarkJobPlanned()
    {
        ...
        $em->expects($this->at(2))->method('getRepository')
            ->with($this->equalTo('MobillogixJobsBundle:JobEvent'))
    ...
    protected function getJobMock($entity = null, $data = [], $type = 'test', $methods = ['getType', 'isNeedsPlanning'])
    {
        ...
        $job = $this->getMockBuilder('Mobillogix\JobsBundle\Tests\Stub\StubJob')
